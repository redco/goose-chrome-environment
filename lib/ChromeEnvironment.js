...
const debug = debugLib('PuppeteerEnvironment');
...
class ChromeEnvironment extends AbstractEnvironment {
  ...
  async _rotateProxy() {
    ...
    let currentProxy = this._proxyCurrent;
    if (!proxy) {
  ...
  _removeUnavailableProxy() {
    ...
    const index = this._proxy.findIndex(item => {
  ...
  async _injectFiles(files) {
    return Promise.all(files.map(file => {
      ...
      return this._injectFile(path.join(__dirname, '..', file));
    }));
  }
